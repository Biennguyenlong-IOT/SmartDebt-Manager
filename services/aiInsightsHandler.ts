  ...
  try {
    ...
    if (!apiKey) {
      return res.status(400).json({
        error: "Chưa cấu hình GEMINI_API_KEY trên Vercel. Vui lòng vào Vercel Project Settings -> Environment Variables, thêm GEMINI_API_KEY và chọn Redeploy."
      });
    }
    ...
    return res.status(500).json({
      error: error?.message || "Lỗi xử lý AI trên máy chủ. Vui lòng thử lại sau."
    });
  }
}
